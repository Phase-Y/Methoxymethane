package me.morishima.methoxymethane.common.items.behaviors;
...
import me.morishima.methoxymethane.api.utils.JsonBlockPattern;
...
/**
 * Copyright by Epimorphism Core (Licensed MIT)
 *
 * @author Gate Guardian, Magic_Sweepy
 */


public class StructureSelectToolBehavior implements IItemBehaviour, ItemUIFactory {

    public static final StructureSelectToolBehavior INSTANCE = new StructureSelectToolBehavior();

    public StructureSelectToolBehavior() {}
    ...
    public ModularUI createUI(PlayerInventoryHolder playerInventoryHolder, EntityPlayer entityPlayer) {
        return ModularUI.builder(GuiTextures.BACKGROUND, 176, 120)
                ...
                .dynamicLabel(15, 13, () -> {
                    ...
                    return String.format("Scale: X-%s  Y-%s  Z-%s", x, y, z);
                }, 0xFAF9F6)
                .widget(new ClickButtonWidget(10, 68, 77, 20, "Export to Log", clickData -> exportLog(playerInventoryHolder)))
                .widget(new ClickButtonWidget(90, 68, 77, 20, "Export to JSON", clickData -> exportLog(playerInventoryHolder)))
                .widget(new ClickButtonWidget(10, 91, 77, 20, "Rotate along Y axis", clickData -> exportLog(playerInventoryHolder)))
                .widget(new ClickButtonWidget(90, 91, 77, 20, "EX", clickData -> exportLog(playerInventoryHolder)))
    ...
    private void exportLog(PlayerInventoryHolder playerInventoryHolder) {
        if (getPos(playerInventoryHolder.getCurrentItem()) != null && !playerInventoryHolder.player.world.isRemote) {
            ...
            DMELog.logger.info(builder.toString());
        }
    }
